...
<div class="gradient-bg min-h-screen relative flex flex-col">
    ...
    <div class="px-5 pt-12 pb-6 flex items-center justify-between">
        <a href="{{ route('transactions.index') }}" class="w-10 h-10 rounded-full bg-white/50 backdrop-blur-md flex items-center justify-center text-gray-700 hover:bg-white/80 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
        </a>
        <h1 class="text-lg font-bold text-[#1a1832]">Edit Transaction</h1>
    ...
    <div class="bg-white rounded-t-[32px] pt-8 px-6 flex-1 pb-24 shadow-[0_-8px_24px_rgba(0,0,0,0.02)] relative z-10">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="category" id="category" class="block w-full rounded-2xl border-gray-200 bg-[#f8f9fc] pl-11 pr-4 py-3.5 text-sm font-medium focus:border-[#8b5cf6] focus:ring focus:ring-[#8b5cf6]/20 transition-shadow appearance-none" required>
                            ...
                            <option value="food" {{ old('category', $transaction->category) == 'food' ? 'selected' : '' }}>🍔 Food & Drink</option>
                            <option value="transport" {{ old('category', $transaction->category) == 'transport' ? 'selected' : '' }}>🚗 Transport</option>
                            <option value="shopping" {{ old('category', $transaction->category) == 'shopping' ? 'selected' : '' }}>🛍️ Shopping</option>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="amount" id="amount" value="{{ old('amount', $transaction->amount) }}" step="0.01" class="block w-full rounded-2xl border-gray-200 bg-[#f8f9fc] pl-11 pr-4 py-3.5 text-lg font-bold text-gray-900 focus:border-[#8b5cf6] focus:ring focus:ring-[#8b5cf6]/20 transition-shadow" required>
                    </div>
                    @error('amount')
                        <div class="text-red-500 text-xs mt-1.5 ml-1 font-medium">{{ $message }}</div>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="transaction_date" id="transaction_date" value="{{ old('transaction_date', $transaction->transaction_date->format('Y-m-d')) }}" class="block w-full rounded-2xl border-gray-200 bg-[#f8f9fc] px-4 py-3.5 text-sm font-medium focus:border-[#8b5cf6] focus:ring focus:ring-[#8b5cf6]/20 transition-shadow" required>
                </div>

                <!-- Catatan -->
                <div>
                    <label for="note" class="block text-[13px] font-bold text-gray-700 mb-1.5 ml-1">Note (Optional)</label>
                    <textarea name="note" id="note" rows="2" class="block w-full rounded-2xl border-gray-200 bg-[#f8f9fc] px-4 py-3.5 text-sm font-medium focus:border-[#8b5cf6] focus:ring focus:ring-[#8b5cf6]/20 transition-shadow">{{ old('note', $transaction->note) }}</textarea>
                </div>

                <div class="pt-4 pb-2">
                    <button type="submit" class="w-full bg-[#8b5cf6] text-white py-4 rounded-2xl font-bold text-sm hover:bg-violet-600 transition-colors shadow-[0_8px_20px_rgba(139,92,246,0.3)] mb-4">
                        Update Transaction
                    </button>
                </div>
            </div>
        ...
        <form action="{{ route('transactions.destroy', $transaction->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="w-full bg-[#fff0f0] text-red-500 py-3.5 rounded-2xl font-bold text-sm hover:bg-red-100 transition-colors" onclick="return confirm('Hapus transaksi ini secara permanen?')">
                Delete Transaction
            </button>
        </form>
    </div>
